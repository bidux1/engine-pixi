...
class Application extends PIXI.Container {
	...
	Resize() {
		if (GameConfig.isMigGamePortrait) {
			...
			this.ratio = this.GAME_HEIGHT / this.GAME_WIDTH;
		...
		this.renderer.view.setAttribute("style", `width:${this.GAME_WIDTH}px; height:${this.GAME_HEIGHT}px`);
		if (window.VPaid) {
			// this.renderer.view.style.width = window.innerWidth;
			this.renderer.view.style.width = this.GAME_WIDTH;
			// this.renderer.view.style.height = window.innerHeight;
			this.renderer.view.style.height = this.GAME_HEIGHT;
